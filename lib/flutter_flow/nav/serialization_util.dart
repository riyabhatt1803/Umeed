...
enum ParamType {
  ...
  FFUploadedFile,
  JSON,
  Document,DocumentReference,DataStruct,
...
dynamic deserializeParam<T>(
  ...
  ParamType paramType,
  bool isList,
  { List<String>? collectionNamePath, StructBuilder<T>? structBuilder, }
) {
  try {
    if (param == null) {
      return null;
    ...
    if (isList) {
      ...
      return paramValues
          .where((p) => p is String)
          .map((p) => p as String)
          .map((p) => deserializeParam<T>(p, paramType, false , collectionNamePath: collectionNamePath, structBuilder: structBuilder,))
          .where((p) => p != null)
          .map((p) => p! as T)
          .toList();
    }
