...
class _FFPlacePickerState extends State<FlutterFlowPlacePicker> {
  ...
  Future displayPrediction(Prediction? p, String? languageCode) async {
    ...
    PlacesDetailsResponse detail = await _places.getDetailsByPlaceId(placeId, language: languageCode);
    setState(() {
    ...
    widget.onSelect(
      FFPlace(
        ...
        state: detail.result.addressComponents
                .firstWhereOrNull((e) =>
                    e.types.contains('administrative_area_level_1'))
                ?.shortName ??
            '',
        ...
        zipCode: detail.result.addressComponents
                .firstWhereOrNull(
                    (e) => e.types.contains('postal_code'))
                ?.shortName ??
            '',
      ),
    );
